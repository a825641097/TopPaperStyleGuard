...
TopPaperLearn 会把你本地投喂的顶级期刊论文语料，转成几类本地文件：
...
仓库内置 Skill：[`skills/top-paper-learn`](skills/top-paper-learn)。
...
Alpha 版本支持 `.txt`、`.md`、`.markdown` 和 `.tex`。PDF 建议先用本地工具提取文本，再交给 `tpl build`。
...
用 `tpl inspect toplearn.profile.json --guard toplearn.guard.json` 可以确认选中的章节、move sequence、n-gram 大小和 guardpack 设置。
